...
export const addItemToRecipe = product => (dispatch, getState) => {
  ...
  axios
    .put(`/recipes/${newRecipe.key}.json?auth=${token}`, newRecipe)
    .then(res => {
...
export const editRecipeItemAmount = (key, amount) => (dispatch, getState) => {
  const token = localStorage.getItem("token");
  let newRecipe = JSON.parse(JSON.stringify(getState().recipe.clickedRecipe));
  newRecipe.items.find(item => item.key === key).quantity = amount;
  axios
    .put(`/recipes/${newRecipe.key}.json?auth=${token}`, newRecipe)
    .then(res => {
...
export const editRecipeName = recipe => dispatch => {
  const token = localStorage.getItem("token");
  axios
    .put(`/recipes/${recipe.key}.json?auth=${token}`, recipe)
    .then(res => {
...
export const deleteRecipe = recipeKey => dispatch => {
  const token = localStorage.getItem("token");
  axios
    .delete(`/recipes/${recipeKey}.json?auth=${token}`)
    .then(() => {
...
export const fetchRecipes = () => dispatch => {
  ...
  axios
    .get(`/recipes.json?auth=${token}&orderBy="userId"&equalTo="${userId}"`)
    .then(res => {
...
export const saveRecipe = recipe => dispatch => {
  ...
  axios
    .post(`/recipes.json?auth=${localStorage.getItem("token")}`, recipe)
    .then(res => {
