...
const AddProduct = () => {
  ...
  const disabledButtonHandler = nutrients => {
    return !!Object.keys(nutrients).find(
      item => nutrients[item].quantity === ""
    );
  };
  ...
  return (
    <>
      ...
      <AddNewProductNutrients
        onchange={nutrientsObj => {
          setNutrients(nutrientsObj);
          setIsButtonDisabled(disabledButtonHandler(nutrientsObj));
        }}
        nutrients={nutrients}
      />
    </>
